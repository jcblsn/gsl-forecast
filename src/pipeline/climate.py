...
the release date, so the directory listing is read to find the current one. Values for a
month are released around the 8th of the following month, one week after the forecast
runs, so the cutoff month is always missing at issue time: these columns serve the fit of
the water balance, not the in-season regressors.
"""
...
def ingest_climdiv(conn, cfg: dict) -> None:
    conn.execute("""
        CREATE TABLE IF NOT EXISTS climdiv_monthly (
            month DATE, division VARCHAR, tavg_f FLOAT, prcp_in FLOAT,
            PRIMARY KEY (month, division)
        )
    """)
    ...
    conn.execute("INSERT OR REPLACE INTO climdiv_monthly SELECT * FROM _climdiv")
